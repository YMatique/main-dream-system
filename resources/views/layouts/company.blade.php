...
<body class="font-sans antialiased bg-gray-50 dark:bg-gray-900">
    <div class="min-h-screen flex">
        ...
        <div id="sidebar" class="hidden lg:flex lg:flex-shrink-0">
            <div class="flex flex-col w-80"> <!-- Aumentado para w-80 -->
                <div class="flex flex-col flex-grow bg-white dark:bg-gray-800 shadow-xl overflow-y-auto">
                    ...
                    <!-- Logo/Company Header -->
                    <div
                        class="flex items-center justify-center h-20 px-6 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-700">
                        <div class="flex items-center space-x-3 w-full">
                            <div
                                class="w-12 h-12 bg-white/20 backdrop-blur-sm rounded-xl flex items-center justify-center ring-2 ring-white/30 shadow-lg">
                                <span
                                    class="text-white font-bold text-xl">{{ substr(Auth::user()->company->name ?? 'E', 0, 1) }}</span>
                            </div>
                            <div class="flex-1 min-w-0">
                                <h2 class="text-white font-bold text-lg truncate">
                                    {{ Auth::user()->company->name ?? 'Empresa' }}</h2>
                    ...
                    <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-600">
                        <div class="flex items-center space-x-3">
                            <div class="relative">
                                ...
                                <div
                                    class="absolute -bottom-1 -right-1 w-4 h-4 bg-green-400 border-2 border-white dark:border-gray-700 rounded-full">
                                </div>
                            </div>
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 truncate">
                                    @if (Auth::user()->user_type === 'company_admin')
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gradient-to-r from-green-100 to-emerald-100 text-green-800 dark:from-green-900 dark:to-emerald-900 dark:text-green-200">
                                            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd"
                                    ...
                                    @else
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gradient-to-r from-blue-100 to-indigo-100 text-blue-800 dark:from-blue-900 dark:to-indigo-900 dark:text-blue-200">
                                            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd"
                    ...
                    <nav class="flex-1 px-4 py-6 space-y-2">
                        ...
                        <!-- Dashboard - Todos podem ver se tiverem login -->
                        @php
                            $user = auth()->user();
                            $dashboardPermission = $user->hasPermission('permission:dashboard');
                            // echo $dashboardPermission?'yes':'no';
                        @endphp
                        @if ( $dashboardPermission)
                            <a href="{{ route('company.dashboard') }}"
                                class="group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 {{ request()->routeIs('company.dashboard')
                                ...
                                <svg class="mr-3 w-5 h-5 {{ request()->routeIs('company.dashboard') ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-500' }}"
                                ...
                                <span>Dashboard</span>
                                <span
                                    class="ml-auto text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-full">Principal</span>
                            </a>
                        @endif

                        <!-- ORDENS DE REPARAÇÃO - Só mostra se tiver acesso a pelo menos um formulário -->
                        @php
                            $user = auth()->user();
                            $hasForm1 = $user->hasPermission('forms.form1.access');
                            $hasForm2 = $user->hasPermission('forms.form2.access');
                            $hasForm3 = $user->hasPermission('forms.form3.access');
                            $hasForm4 = $user->hasPermission('forms.form4.access');
                            $hasForm5 = $user->hasPermission('forms.form5.access');

                            $hasAnyForm = $hasForm1 || $hasForm2 || $hasForm3 || $hasForm4 || $hasForm5;
                            $hasOrdersListAccess = auth()->user()->hasPermission('repair_orders.view_all');
                        @endphp
                        @if ($hasAnyForm || $hasOrdersListAccess)
                            ...
                            <button @click="ordersOpen = !ordersOpen" 
                                    class="w-full group flex items-center justify-between px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 text-gray-600 hover:bg-gray-50 hover:text-gray-900">
                                ...
                                <!-- Submenu: Formulários -->
                                <div x-show="ordersOpen" x-transition:enter="transition ease-out duration-200"
                                    x-transition:enter-start="opacity-0 transform -translate-y-2"
                                    x-transition:enter-end="opacity-100 transform translate-y-0" class="ml-6 space-y-1">
                                    @if ($hasAnyForm)
                                        <div
                                            class="text-xs font-semibold text-gray-400 uppercase tracking-wider px-4 py-2 flex items-center">
                      ...
                      {{ request()->routeIs('company.repair-orders.form1')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.repair-orders.form1') ? 'bg-blue-500' : 'bg-gray-300 group-hover:bg-blue-500' }}">
                                            </div>
                      ...
                      {{ request()->routeIs('company.repair-orders.form2')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.repair-orders.form2') ? 'bg-blue-500' : 'bg-gray-300 group-hover:bg-blue-500' }}">
                                            </div>
                      ...
                      {{ request()->routeIs('company.repair-orders.form3')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.repair-orders.form3') ? 'bg-blue-500' : 'bg-gray-300 group-hover:bg-blue-500' }}">
                                            </div>
                      ...
                      {{ request()->routeIs('company.repair-orders.form4')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.repair-orders.form4') ? 'bg-blue-500' : 'bg-gray-300 group-hover:bg-blue-500' }}">
                                            </div>
                      ...
                      {{ request()->routeIs('company.repair-orders.form5')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.repair-orders.form5') ? 'bg-blue-500' : 'bg-gray-300 group-hover:bg-blue-500' }}">
                                            </div>
                                    ...
                                    <!-- Submenu: Listagens - Só mostra se tiver permissão para listar -->
                                    ...
                                    @if ($hasAnyFormList || $hasOrdersListAccess)
                                        <div
                                            class="text-xs font-semibold text-gray-400 uppercase tracking-wider px-4 py-2 mt-3 flex items-center">
                                            <svg class="w-3 h-3 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd"
                                    ...
                                    @if ($hasAnyFormList || $hasOrdersListAccess)
                                        <a href="{{ route('company.orders.index') }}"
                                            class="group flex items-center px-4 py-2.5 text-sm rounded-lg transition-all duration-200
                      {{ request()->routeIs('company.orders.index')
                          ? 'bg-purple-50 text-purple-700 border-l-2 border-purple-500 dark:bg-purple-900/20'
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.orders.index') ? 'bg-purple-500' : 'bg-gray-300 group-hover:bg-purple-500' }}">
                                            </div>
                                            <span class="flex-1">Listagem Geral</span>
                                            <span class="text-xs text-gray-400">Geral</span>
                                        </a>
                                    @endif

                                    @if($hasOrdersListAccess)
                      ...
                      {{ request()->routeIs('company.orders.advanced-list')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.orders.advanced-list') ? 'bg-purple-500' : 'bg-gray-300 group-hover:bg-purple-500' }}">
                                            </div>
                                            <span class="flex-1">Listagem Avançada</span>
                                            <span
                                                class="text-xs bg-gradient-to-r from-purple-100 to-pink-100 text-purple-800 px-2 py-0.5 rounded-full">Pro</span>
                                        </a>
                                    @endif
                                </div>
                            </div>
                        ...
                        <!-- FATURAÇÃO - Só mostra se tiver permissões de billing -->
                        @php
                            $hasBillingAccess =
                                auth()->user()->hasPermission('billing.hh.manage') ||
                                auth()->user()->hasPermission('billing.estimated.manage') ||
                                auth()->user()->hasPermission('billing.real.manage') ||
                                auth()->user()->hasPermission('billing.view_all');
                        @endphp
                       ...
                       {{ request()->routeIs('company.billing.*') ? 'bg-gradient-to-r from-green-50 to-emerald-50 text-green-700 dark:from-green-900/20 dark:to-emerald-900/20 dark:text-green-300' : '' }}">
                                    <div class="flex items-center">
                                        <svg class="mr-3 w-5 h-5 {{ request()->routeIs('company.billing.*') ? 'text-green-600' : 'text-gray-400 group-hover:text-gray-500' }}"
                                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <div class="flex items-center space-x-2">
                                        <span
                                            class="text-xs bg-green-100 text-green-600 px-2 py-1 rounded-full">{{ collect(['billing.real.manage', 'billing.estimated.manage', 'billing.hh.manage'])->filter(fn($perm) => auth()->user()->hasPermission($perm))->count() }}</span>
                                        <svg class="w-4 h-4 transition-transform duration-200"
                                            :class="billingOpen ? 'rotate-180' : ''" fill="none"
                                ...
                                <div x-show="billingOpen" x-transition:enter="transition ease-out duration-200"
                                    x-transition:enter-start="opacity-0 transform -translate-y-2"
                                    x-transition:enter-end="opacity-100 transform translate-y-0"
                                    class="ml-6 space-y-1">
                                    @if( auth()->user()->hasPermission('billing.real.manage') || auth()->user()->hasPermission('billing.view_all'))
                      ...
                      {{ request()->routeIs('company.billing.real')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.billing.real') ? 'bg-green-500' : 'bg-gray-300 group-hover:bg-green-500' }}">
                                            </div>
                                    ...
                                    @endif
                                      ...
                                      @if( auth()->user()->hasPermission('billing.estimated.manage') || auth()->user()->hasPermission('billing.view_all'))
                      ...
                      {{ request()->routeIs('company.billing.estimated')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.billing.estimated') ? 'bg-yellow-500' : 'bg-gray-300 group-hover:bg-yellow-500' }}">
                                            </div>
                                    ...
                                    @endif
                                      ...
                                      @if( auth()->user()->hasPermission('billing.hh.manage') || auth()->user()->hasPermission('billing.view_all'))
                      ...
                      {{ request()->routeIs('company.billing.hh')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.billing.hh') ? 'bg-indigo-500' : 'bg-gray-300 group-hover:bg-indigo-500' }}">
                                            </div>
                        ...
                        <!-- GESTÃO - Só mostra se tiver permissões de masters -->
                        @php
                            $hasMastersAccess = collect([
                            ...
                            ])->some(fn($perm) => auth()->user()->hasPermission($perm));
                        @endphp
                       ...
                       {{ request()->routeIs('company.manage.*') ? 'bg-gradient-to-r from-blue-50 to-cyan-50 text-blue-700 dark:from-blue-900/20 dark:to-cyan-900/20 dark:text-blue-300' : '' }}">
                                    <div class="flex items-center">
                                        <svg class="mr-3 w-5 h-5 {{ request()->routeIs('company.manage.*') ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-500' }}"
                                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <div class="flex items-center space-x-2">
                                        @php
                                            $mastersCount = collect([
                                                ...
                                                ->filter(fn($perm) => auth()->user()->hasPermission($perm))
                                                ->count();
                                        @endphp
                                        <span
                                            class="text-xs bg-blue-100 text-blue-600 px-2 py-1 rounded-full">{{ $mastersCount }}</span>
                                        <svg class="w-4 h-4 transition-transform duration-200"
                                            :class="manageOpen ? 'rotate-180' : ''" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M19 9l-7 7-7-7"></path>
                                        </svg>
                                ...
                                <div x-show="manageOpen" x-transition:enter="transition ease-out duration-200"
                                    x-transition:enter-start="opacity-0 transform -translate-y-2"
                                    x-transition:enter-end="opacity-100 transform translate-y-0"
                                    class="ml-6 space-y-1">
                                    @if(auth()->user()->hasPermission('masters.employees.manage'))
                      ...
                      {{ request()->routeIs('company.manage.employees')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.manage.employees') ? 'bg-blue-500' : 'bg-gray-300 group-hover:bg-blue-500' }}">
                                            </div>
                                            ...
                                            <span
                                                class="text-xs text-gray-400">{{ \App\Models\Company\Employee::where('company_id', auth()->user()->company_id)->count() ?? 0 }}</span>
                                        </a>
                                    @endif
                                    @if(auth()->user()->hasPermission('masters.clients.manage'))
                      ...
                      {{ request()->routeIs('company.manage.clients')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.manage.clients') ? 'bg-blue-500' : 'bg-gray-300 group-hover:bg-blue-500' }}">
                                            </div>
                                            <svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            <span
                                                class="text-xs text-gray-400">{{ \App\Models\Company\Client::where('company_id', auth()->user()->company_id)->count() ?? 0 }}</span>
                                        </a>
                                    @endif

                                    @if(auth()->user()->hasPermission('masters.materials.manage'))
                      ...
                      {{ request()->routeIs('company.manage.materials')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.manage.materials') ? 'bg-blue-500' : 'bg-gray-300 group-hover:bg-blue-500' }}">
                                            </div>
                                            <svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            <span
                                                class="text-xs text-gray-400">{{ \App\Models\Company\Material::where('company_id', auth()->user()->company_id)->count() ?? 0 }}</span>
                                        </a>
                                    @endif
                                    @if(auth()->user()->hasPermission('masters.departments.manage'))
                      ...
                      {{ request()->routeIs('company.manage.departments')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.manage.departments') ? 'bg-blue-500' : 'bg-gray-300 group-hover:bg-blue-500' }}">
                                            </div>
                                            <svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            <span
                                                class="text-xs text-gray-400">{{ \App\Models\Company\Department::where('company_id', auth()->user()->company_id)->count() ?? 0 }}</span>
                                        </a>
                                    @endif
                                    @if(auth()->user()->hasPermission('masters.maintenance_types.manage'))
                      ...
                      {{ request()->routeIs('company.manage.maintenance-types')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.manage.maintenance-types') ? 'bg-blue-500' : 'bg-gray-300 group-hover:bg-blue-500' }}">
                                            </div>
                                            <svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    @endif
                                    @if(auth()->user()->hasPermission('masters.costs.manage'))
                      ...
                      {{ request()->routeIs('company.manage.client-costs')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.manage.client-costs') ? 'bg-blue-500' : 'bg-gray-300 group-hover:bg-blue-500' }}">
                                            </div>
                                            <svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    @endif
                                    @if(auth()->user()->hasPermission('masters.machines.manage'))
                      ...
                      {{ request()->routeIs('company.manage.machine-numbers')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.manage.machine-numbers') ? 'bg-blue-500' : 'bg-gray-300 group-hover:bg-blue-500' }}">
                                            </div>
                                            <svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    @endif
                                    @if(auth()->user()->hasPermission('masters.requesters.manage'))
                      ...
                      {{ request()->routeIs('company.manage.requesters')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.manage.requesters') ? 'bg-blue-500' : 'bg-gray-300 group-hover:bg-blue-500' }}">
                                            </div>
                                            <svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    @endif
                                    @if(auth()->user()->hasPermission('masters.statuses.manage'))
                      ...
                      {{ request()->routeIs('company.manage.statuses')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.manage.statuses') ? 'bg-blue-500' : 'bg-gray-300 group-hover:bg-blue-500' }}">
                                            </div>
                                            <svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    @endif
                                    @if(auth()->user()->hasPermission('masters.users.manage'))
                      ...
                      {{ request()->routeIs('company.manage.users-permissions')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.manage.users-permissions') ? 'bg-blue-500' : 'bg-gray-300 group-hover:bg-blue-500' }}">
                                            </div>
                                            <svg class="w-4 h-4 mr-2 text-gray-400" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <!-- AVALIAÇÃO - Só mostra se tiver permissões de evaluation -->
                        @php
                            $hasEvaluationAccess = collect([
                                'evaluation.create',
                                'evaluation.approve',
                                'evaluation.reports',
                                'evaluation.metrics',
                        ...
                        @if ($hasEvaluationAccess)
                            <div class="space-y-1" x-data="{ evaluationOpen: {{ request()->routeIs('company.performance.*') ? 'true' : 'false' }} }">
                       ...
                       {{ request()->routeIs('company.performance.*') ? 'bg-gradient-to-r from-orange-50 to-red-50 text-orange-700 dark:from-orange-900/20 dark:to-red-900/20 dark:text-orange-300' : '' }}">
                                    <div class="flex items-center">
                                        <svg class="mr-3 w-5 h-5 {{ request()->routeIs('company.performance.*') ? 'text-orange-600' : 'text-gray-400 group-hover:text-gray-500' }}"
                                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <div class="flex items-center space-x-2">
                                        ...
                                        @endphp
                                        <span
                                            class="text-xs bg-orange-100 text-orange-600 px-2 py-1 rounded-full">{{ $evaluationCount }}</span>
                                        <svg class="w-4 h-4 transition-transform duration-200"
                                            :class="evaluationOpen ? 'rotate-180' : ''" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M19 9l-7 7-7-7"></path>
                                        </svg>
                                ...
                                <div x-show="evaluationOpen" x-transition:enter="transition ease-out duration-200"
                                    x-transition:enter-start="opacity-0 transform -translate-y-2"
                                    x-transition:enter-end="opacity-100 transform translate-y-0"
                                    class="ml-6 space-y-1">
                      ...
                      {{ request()->routeIs('company.performance.metrics')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.performance.metrics') ? 'bg-orange-500' : 'bg-gray-300 group-hover:bg-orange-500' }}">
                                            </div>
                      ...
                      {{ request()->routeIs('company.performance.evaluations.stages')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.performance.evaluations.stages') ? 'bg-orange-500' : 'bg-gray-300 group-hover:bg-orange-500' }}">
                                            </div>
                                            <span class="flex-1">Gestão de Estágios</span>
                                            <span class="text-xs text-orange-600 font-medium">GE</span>
                                        </a>
                                    @endif
                                    @if(auth()->user()->hasPermission('evaluation.create'))
                      ...
                      {{ request()->routeIs('company.performance.evaluations')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.performance.evaluations') ? 'bg-orange-500' : 'bg-gray-300 group-hover:bg-orange-500' }}">
                                            </div>
                                            <span class="flex-1">Avaliar Funcionários</span>
                                            <span class="text-xs text-orange-600 font-medium">Eval</span>
                                        </a>
                                    @endif
                                    @if(auth()->user()->hasPermission('evaluation.approve'))
                      ...
                      {{ request()->routeIs('company.performance.evaluations.approvals')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.performance.evaluations.approvals') ? 'bg-orange-500' : 'bg-gray-300 group-hover:bg-orange-500' }}">
                                            </div>
                                            <span class="flex-1">Aprovação de Funcionários</span>
                                            <span class="text-xs text-orange-600 font-medium">Apr</span>
                                        </a>
                                    @endif
                                    @if(auth()->user()->hasPermission('evaluation.reports'))
                      ...
                      {{ request()->routeIs('company.performance.reports')
                          ...
                          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700' }}">
                                            <div
                                                class="w-2 h-2 rounded-full mr-3 {{ request()->routeIs('company.performance.reports') ? 'bg-orange-500' : 'bg-gray-300 group-hover:bg-blue-500' }}">
                                            </div>
                    ...
                    <div class="p-6 border-t border-gray-200 dark:border-gray-600">
                        <a href="{{ route('company.my-permissions') }}"
                            class="w-full mb-4 group flex items-center px-4 py-3 text-sm font-medium text-gray-600 dark:text-gray-400 rounded-xl hover:bg-blue-50 hover:text-blue-700 dark:hover:bg-blue-900/20 transition-all duration-200 border border-gray-200 dark:border-gray-600 hover:border-red-200">
                        ...
                        <form method="POST" action="{{ route('company.logout') }}" class="w-full">
                            @csrf
                            <button type="submit"
                                class="w-full group flex items-center px-4 py-3 text-sm font-medium text-gray-600 dark:text-gray-400 rounded-xl hover:bg-red-50 hover:text-red-700 dark:hover:bg-red-900/20 transition-all duration-200 border border-gray-200 dark:border-gray-600 hover:border-red-200">
                                <svg class="mr-3 w-5 h-5 group-hover:text-red-600" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="flex flex-col w-0 flex-1 overflow-hidden">
            ...
            <header
                class="relative  bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between h-20 px-6 lg:px-8">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <button id="open-sidebar"
                            class="lg:hidden p-2 rounded-lg text-gray-500 hover:text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-gray-300 dark:hover:bg-gray-700 transition-colors">
                        ...
                        <div class="relative" x-data="{ open: false, tooltip: false }">
                            ...
                            <div x-show="open" @click.away="open = false"
                                ...
                                <div class="max-h-64 overflow-y-auto">
                                    ...
                                    <div
                                        ...
                                        <div class="flex items-start space-x-3">
                                            ...
                                            <div class="flex-1">
                                                <p class="text-sm text-gray-900 dark:text-white">Nova ordem de
                                                    reparação #1234</p>
                                                <p class="text-xs text-gray-500 dark:text-gray-400">Cliente: Empresa
                                                    XYZ - Há 2 horas</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div
                                        ...
                                        <div class="flex items-start space-x-3">
                                            ...
                                            <div class="flex-1">
                                                ...
                                                <p class="text-xs text-gray-500 dark:text-gray-400">Ordem #1230 - Há 1
                                                    dia</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="p-3 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer">
                                        <div class="flex items-start space-x-3">
                                            ...
                                            <div class="flex-1">
                                                <p class="text-sm text-gray-900 dark:text-white">Material em baixo
                                                    stock</p>
                                                <p class="text-xs text-gray-500 dark:text-gray-400">Parafusos M8 - Há 2
                                                    dias</p>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="p-3 border-t border-gray-200 dark:border-gray-700">
                                    <a href="#" class="text-sm text-blue-600 hover:text-blue-500">Ver todas as
                                        notificações</a>
                                </div>
                            </div>
                        </div>
                        ...
                        <div
                            ...
                            <div class="text-right">
                                <p class="text-sm font-medium text-gray-900 dark:text-white">
                                    {{ Auth::user()->company->name ?? 'Empresa' }}</p>
                            ...
                            <div
                                ...
                                <span
                                    class="text-white font-bold text-sm">{{ substr(Auth::user()->company->name ?? 'E', 0, 1) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                @if (isset($breadcrumbs) || request()->route())
                    <div
                        class="border-t border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/30 px-6 lg:px-8 py-3">
                            ...
                            <ol class="flex items-center space-x-2 text-sm">
                                ...
                                @php
                                    ...
                                    $breadcrumbMap = [
                                        ...
                                        'company.manage.maintenance-types' => [
                                            ['Gestão', null],
                                            ['Tipos de Manutenção', null],
                                        ],
                                        'company.manage.client-costs' => [
                                            ['Gestão', null],
                                            ['Custos de Clientes', null],
                                        ],
                                        'company.manage.machine-numbers' => [
                                            ['Gestão', null],
                                            ['Números de Máquina', null],
                                        ],
                                        'company.manage.requesters' => [['Gestão', null], ['Solicitantes', null]],
                                        'company.manage.statuses' => [
                                            ['Gestão', null],
                                            ['Estados & Localização', null],
                                        ],
                                        'company.manage.users-permissions' => [
                                            ['Gestão', null],
                                            ['Utilizadores & Permissões', null],
                                        ],
                                        ...
                                        'company.orders.advanced-list' => [
                                            ['Ordens', null],
                                            ['Listagem Avançada', null],
                                        ],
                                        'company.performance.metrics' => [
                                            ['Avaliação', null],
                                            ['Gestão de Métricas', null],
                                        ],
                                        'company.performance.perform' => [
                                            ['Avaliação', null],
                                            ['Avaliar Funcionários', null],
                                        ],
                                ...
                                @foreach ($currentBreadcrumbs as $index => $breadcrumb)
                                    <li class="flex items-center">
                                        <svg class="w-4 h-4 text-gray-400 mx-2" fill="currentColor"
                                            viewBox="0 0 20 20">
                                            <path fill-rule="evenodd"
                                        ...
                                        @if ($index === count($currentBreadcrumbs) - 1)
                                            <span
                                                class="text-gray-900 dark:text-white font-medium">{{ $breadcrumb[0] }}</span>
                                        @else
                                            <a href="{{ $breadcrumb[1] ?? '#' }}"
                                                class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 transition-colors">
                                                {{ $breadcrumb[0] }}
    ...
    <div id="sidebar-overlay" class="hidden fixed inset-0 bg-gray-600 bg-opacity-75 z-40 lg:hidden">
        <div id="mobile-sidebar"
            class="fixed inset-y-0 left-0 flex flex-col w-80 bg-white dark:bg-gray-800 shadow-xl transform -translate-x-full transition-transform duration-300 ease-in-out">
            <!-- Same sidebar content as desktop but with close button -->
            <div
                class="flex items-center justify-between h-16 px-6 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-700">
                <div class="flex items-center space-x-3">
                    <div
                        class="w-10 h-10 bg-white/20 backdrop-blur-sm rounded-xl flex items-center justify-center ring-2 ring-white/30 shadow-lg">
                        <span
                            class="text-white font-bold text-lg">{{ substr(Auth::user()->company->name ?? 'E', 0, 1) }}</span>
                    </div>
                    <div>
