<div class="space-y-8 p-4 sm:p-6">
    ...
    <div class="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-xl shadow-lg p-6 text-white">
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
            <div>
                <h2 class="text-3xl font-bold tracking-tight">Ordens de Reparação - Formulário 4</h2>
                <p class="mt-2 text-sm text-blue-100">Listagem de ordens com número de máquina</p>
            </div>
            <div class="flex flex-col sm:flex-row gap-3">
                ...
                @if ($hasPermissionToCreate)
                    <a href="{{ route('company.orders.form4') }}" class="px-5 py-2.5 bg-white text-blue-700 rounded-lg hover:bg-blue-50 transition-all duration-300 flex items-center shadow-sm">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
                ...
                <div class="relative">
                    <input wire:model.live.debounce.300ms="search" type="text" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-300" placeholder="Ordem, nº máquina...">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach ($form4s as $form4)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-200">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900 dark:text-white">{{ $form4->repairOrder?->order_number ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $form4->carimbo?->format('d/m/Y H:i') ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $form4->location?->name ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $form4->status?->name ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $form4->machine_number ?? $form4->repairOrder?->machineNumber->number ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <button wire:click="viewOrder({{ $form4->id }})" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 mr-4 transition-colors duration-200 flex items-center">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <button wire:click="continueOrder({{ $form4->id }})" class="text-green-600 hover:text-green-800 dark:text-green-400 dark:hover:text-green-300 transition-colors duration-200 flex items-center">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach ($form4s as $form4)
                        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 p-6 transition-all duration-300 hover:shadow-lg">
                            <div class="flex justify-between items-center mb-4">
                                <span class="text-lg font-bold text-gray-900 dark:text-white">{{ $form4->repairOrder?->order_number ?? '-' }}</span>
                                <span class="text-sm text-gray-600 dark:text-gray-400">{{ $form4->carimbo?->format('d/m/Y') ?? '-' }}</span>
                            </div>
                            <div class="space-y-3 mb-4">
                                <div class="flex justify-between">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">Localização:</span>
                                    <span class="text-sm font-bold text-gray-900 dark:text-white">{{ $form4->location?->name ?? '-' }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">Status:</span>
                                    <span class="text-sm font-bold text-gray-900 dark:text-white">{{ $form4->status?->name ?? '-' }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">Nº Máquina:</span>
                                    <span class="text-sm font-bold text-gray-900 dark:text-white">{{ $form4->machine_number ?? $form4->repairOrder?->machine_number ?? '-' }}</span>
                                </div>
                            </div>
                            <div class="flex justify-between items-center">
                                <button wire:click="viewOrder({{ $form4->id }})" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 text-sm font-medium transition-colors duration-200 flex items-center">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                    </svg>
                                ...
                                <button wire:click="continueOrder({{ $form4->id }})" class="px-4 py-2 bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400 rounded-lg text-sm font-medium hover:bg-green-200 dark:hover:bg-green-900/40 transition-all duration-300 flex items-center">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 5l7 7-7 7M5 5l7 7-7 7"></path>
                                    </svg>
                                    Continuar
    ...
    @else
        <div class="text-center py-12">
            ...
            <p class="text-gray-600 dark:text-gray-400 mb-4">
                @if ($activeFiltersCount > 0)
                    Ajuste os filtros para encontrar ordens no Formulário 4.
                @else
                    Nenhuma ordem avançou para o Formulário 4. Crie ou continue uma ordem.
                @endif
            </p>
            <div class="flex flex-col sm:flex-row gap-3 justify-center items-center">
                ...
                @endif
                @if ($hasPermissionToCreate)
                    <a href="{{ route('company.orders.form4') }}" class="px-5 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all duration-300 flex items-center shadow-sm">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                        </svg>
                        Continuar Ordem
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                window.addEventListener('show-order-details', function(event) {
                    const data = event.detail[0].form4Data;
                    ...
                    modal.innerHTML = `
                        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-lg w-full shadow-xl max-h-[80vh] overflow-y-auto">
                            ...
                            <div class="space-y-4 text-sm text-gray-700 dark:text-gray-300">
                                ...
                                <p><strong>Nº Máquina:</strong> ${data.machine_number || data.machine_number.number || '-'}</p>
                            </div>
