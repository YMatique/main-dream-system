<div class="space-y-8 p-4 sm:p-6">
    ...
    <div class="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-xl shadow-lg p-6 text-white">
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div>
                <h2 class="text-3xl font-extrabold tracking-tight">Ordens de Reparação - Formulário 1</h2>
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                ...
                @if ($this->hasPermissionToCreate)
                    <a href="{{ route('company.orders.form1-list') }}" class="px-5 py-2.5 bg-white text-blue-700 rounded-lg hover:bg-blue-50 transition-all duration-300 flex items-center shadow-sm">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 p-5 transition-all duration-300 hover:shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ number_format($metrics['total_orders']) }}</p>
                    </div>
                ...
                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">{{ $metrics['period_start'] }} - {{ $metrics['period_end'] }}</p>
            </div>
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 p-5 transition-all duration-300 hover:shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Ordens por Tipo</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $metrics['maintenance_types_count'] ?? 0 }}</p>
                    </div>
                    <div class="p-3 bg-green-100 dark:bg-green-900/30 rounded-full">
                        <svg class="w-6 h-6 text-green-600 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4m-12 0l4-4-4-4"></path>
                        </svg>
                    </div>
                </div>
                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">Tipos de manutenção ativos</p>
            </div>
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 p-5 transition-all duration-300 hover:shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Ordens Avançadas</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $metrics['advanced_orders'] ?? 0 }}</p>
                    </div>
                ...
                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">Ordens que passaram para Form2+</p>
            </div>
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 p-5 transition-all duration-300 hover:shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Ordens Completas</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $metrics['completed_orders'] ?? 0 }}</p>
                    </div>
                ...
                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">Ordens finalizadas</p>
            </div>
        </div>
    @endif
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
                ...
                <div class="relative">
                    <input wire:model.live.debounce.300ms="search" type="text" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-300" placeholder="Ordem, cliente, máquina...">
            ...
            <div>
                ...
                <select wire:model.live="filterByMachine" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-300">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            <th wire:click="sortBy('order_number')" class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 transition-all duration-200">
                                Ordem @if ($sortField === 'order_number') {{ $sortDirection === 'asc' ? '↑' : '↓' }} @endif
                            </th>
                            <th wire:click="sortBy('form1.client.name')" class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 transition-all duration-200">
                                Cliente @if ($sortField === 'form1.client.name') {{ $sortDirection === 'asc' ? '↑' : '↓' }} @endif
                            </th>
                            <th wire:click="sortBy('form1.maintenanceType.name')" class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 transition-all duration-200">
                                Tipo Manutenção @if ($sortField === 'form1.maintenanceType.name') {{ $sortDirection === 'asc' ? '↑' : '↓' }} @endif
                            </th>
                            <th wire:click="sortBy('form1.machineNumber.number')" class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 transition-all duration-200">
                                Máquina @if ($sortField === 'form1.machineNumber.number') {{ $sortDirection === 'asc' ? '↑' : '↓' }} @endif
                            </th>
                            <th wire:click="sortBy('form1.carimbo')" class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 transition-all duration-200">
                                Data @if ($sortField === 'form1.carimbo') {{ $sortDirection === 'asc' ? '↑' : '↓' }} @endif
                            </th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Descrição Avaria</th>
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach ($orders as $order)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-200">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $order->form1?->client?->name ?? '-' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $order->form1?->maintenanceType?->name ?? '-' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $order->form1?->machineNumber?->number ?? '-' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $order->form1?->carimbo->format('d/m/Y H:i') ?? '-' }}</td>
                                <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">{{ Str::limit($order->form1?->descricao_avaria ?? '-', 50) }}</td>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach ($orders as $order)
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 p-6 transition-all duration-300 hover:shadow-lg">
                        <div class="flex justify-between items-center mb-4">
                            <span class="text-lg font-semibold text-gray-900 dark:text-white">{{ $order->order_number }}</span>
                            <span class="text-sm text-gray-600 dark:text-gray-400">{{ $order->form1?->carimbo->format('d/m/Y') ?? '-' }}</span>
                        </div>
                        <div class="space-y-3 mb-4">
                            <div class="flex justify-between">
                                <span class="text-sm text-gray-600 dark:text-gray-400">Cliente:</span>
                                <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $order->form1?->client?->name ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-sm text-gray-600 dark:text-gray-400">Tipo Manutenção:</span>
                                <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $order->form1?->maintenanceType?->name ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-sm text-gray-600 dark:text-gray-400">Máquina:</span>
                                <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $order->form1?->machineNumber?->number ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-sm text-gray-600 dark:text-gray-400">Descrição:</span>
                                <span class="text-sm font-medium text-gray-900 dark:text-white">{{ Str::limit($order->form1?->descricao_avaria ?? '-', 30) }}</span>
                            </div>
                        </div>
        ...
        <div class="mt-6 flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            @if ($this->hasPermissionToExport)
                <div class="flex gap-2">
                    <button wire:click="exportOrders('excel')" class="px-5 py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-all duration-300 shadow-sm">Exportar Excel</button>
                    <button wire:click="exportOrders('csv')" class="px-5 py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-all duration-300 shadow-sm">Exportar CSV</button>
                    <button wire:click="exportOrders('pdf')" class="px-5 py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-all duration-300 shadow-sm">Exportar PDF</button>
                </div>
            @endif
        </div>
    @else
        <div class="text-center py-12">
            ...
            <h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-2">Nenhuma ordem encontrada</h3>
            ...
            <div class="flex flex-col sm:flex-row gap-3 justify-center items-center">
                ...
                @endif
                @if ($this->hasPermissionToCreate)
                    <a href="{{ route('company.orders.form1-list') }}" class="px-5 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all duration-300 flex items-center shadow-sm">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                        </svg>
    ...
    {{-- Loading Overlay --}}
    <div wire:loading.flex wire:target="search,filterByClient,filterByMaintenanceType,filterByMonthYear,filterByMachine,filterStartDate,filterEndDate,sortBy,continueOrder,viewOrder,refreshData,exportOrders" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
            ...
            window.addEventListener('show-order-details', function(event) {
                ...
                modal.innerHTML = `
                    <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-lg w-full shadow-xl">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Detalhes da Ordem #${event.detail.orderId}</h3>
