<div class="space-y-8 p-4 sm:p-6">
    ...
    <div class="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-xl shadow-lg p-6 text-white">
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
            <div>
                <h2 class="text-3xl font-bold tracking-tight">Ordens de Reparação - Formulário 3</h2>
                <p class="mt-2 text-sm text-blue-100">Listagem de ordens com faturação e materiais</p>
            </div>
            <div class="flex flex-col sm:flex-row gap-3">
                ...
                @if ($hasPermissionToCreate)
                    <a href="{{ route('company.orders.form3') }}" class="px-5 py-2.5 bg-white text-blue-700 rounded-lg hover:bg-blue-50 transition-all duration-300 flex items-center shadow-sm">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if ($showMetrics)
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div class="bg-white/10 backdrop-blur-sm rounded-lg p-4 text-white">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-white/80">Ordens com Materiais</p>
                            <p class="text-2xl font-bold">{{ $metrics['orders_with_materials'] ?? 0 }}</p>
                        </div>
                        <svg class="w-8 h-8 text-white/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"></path>
                        </svg>
                    </div>
                    <p class="mt-2 text-xs text-white/60">Ordens com materiais alocados</p>
                </div>
                <div class="bg-white/10 backdrop-blur-sm rounded-lg p-4 text-white">
                    <div class="flex items-center justify-between">
                        <div>
                ...
                <div class="bg-white/10 backdrop-blur-sm rounded-lg p-4 text-white">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-white/80">Total Horas Faturadas</p>
                    ...
                    <p class="mt-2 text-xs text-white/60">Horas faturadas no período</p>
                </div>
            </div>
        @endif
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
                ...
                <div class="relative">
                    <input wire:model.live.debounce.300ms="search" type="text" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-300" placeholder="Ordem, nº máquina, material...">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                ...
                                <th wire:click="sortBy('billing_date')" class="px-6 py-4 text-left text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 transition-all duration-200">
                                    Data Faturação @if ($sortField === 'billing_date') {{ $sortDirection === 'asc' ? '↑' : '↓' }} @endif
                                </th>
                                ...
                                <th wire:click="sortBy('billed_hours')" class="px-6 py-4 text-left text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 transition-all duration-200">
                                    Horas Faturadas @if ($sortField === 'billed_hours') {{ $sortDirection === 'asc' ? '↑' : '↓' }} @endif
                                </th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Materiais</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Nº Máquina</th>
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach ($form3s as $form3)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-200">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900 dark:text-white">{{ $form3->repairOrder?->order_number ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $form3->carimbo?->format('d/m/Y H:i') ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $form3->data_faturacao->format('d/m/Y') ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $form3->location?->name ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $form3->status?->name ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ number_format($form3->horas_faturadas ?? 0, 2) }}h</td>
                                    <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">{{ $form3->materials->isNotEmpty() ? $form3->materials->map(function ($m) { return $m->name . ' (' . $m->quantidade . ')'; })->implode(', ') : '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $form3->repairOrder?->form1->numberNumber->number?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <button wire:click="viewOrder({{ $form3->id }})" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 mr-4 transition-colors duration-200 flex items-center">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <button disabled class="text-gray-400 dark:text-gray-600 mr-4 flex items-center cursor-not-allowed">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                            </svg>
                                            Editar
                                        </button>
                                        <button wire:click="continueOrder({{ $form3->id }})" class="text-green-600 hover:text-green-800 dark:text-green-400 dark:hover:text-green-300 transition-colors duration-200 flex items-center">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach ($form3s as $form3)
                        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 p-6 transition-all duration-300 hover:shadow-lg">
                            <div class="flex justify-between items-center mb-4">
                                <span class="text-lg font-bold text-gray-900 dark:text-white">{{ $form3->repairOrder?->order_number ?? '-' }}</span>
                                <span class="text-sm text-gray-600 dark:text-gray-400">{{ $form3->carimbo?->format('d/m/Y') ?? '-' }}</span>
                            </div>
                            <div class="space-y-3 mb-4">
                                <div class="flex justify-between">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">Data Faturação:</span>
                                    <span class="text-sm font-bold text-gray-900 dark:text-white">{{ $form3->data_faturacao?->format('d/m/Y') ?? '-' }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">Localização:</span>
                                    <span class="text-sm font-bold text-gray-900 dark:text-white">{{ $form3->location?->name ?? '-' }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">Status:</span>
                                    <span class="text-sm font-bold text-gray-900 dark:text-white">{{ $form3->status?->name ?? '-' }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">Horas Faturadas:</span>
                                    <span class="text-sm font-bold text-gray-900 dark:text-white">{{ number_format($form3->horas_faturadas ?? 0, 2) }}h</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">Materiais:</span>
                                    <span class="text-sm font-bold text-gray-900 dark:text-white">{{ $form3->materials->isNotEmpty() ? $form3->materials->map(function ($m) { return $m->material->name . ' (' . $m->quantidade . ')'; })->implode(', ') : '-' }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">Nº Máquina:</span>
                                    <span class="text-sm font-bold text-gray-900 dark:text-white">{{ $form3->repairOrder?->form1->machineNumber->number ?? '-' }}</span>
                                </div>
                            </div>
                            <div class="flex justify-between items-center">
                                <button wire:click="viewOrder({{ $form3->id }})" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 text-sm font-medium transition-colors duration-200 flex items-center">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                    </svg>
                                ...
                                <button wire:click="continueOrder({{ $form3->id }})" class="px-4 py-2 bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400 rounded-lg text-sm font-medium hover:bg-green-200 dark:hover:bg-green-900/40 transition-all duration-300 flex items-center">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 5l7 7-7 7M5 5l7 7-7 7"></path>
                                    </svg>
                                    Continuar
    ...
    @else
        <div class="text-center py-12">
            ...
            <p class="text-gray-600 dark:text-gray-400 mb-4">
                @if ($activeFiltersCount > 0)
                    Ajuste os filtros para encontrar ordens no Formulário 3.
                @else
                    Nenhuma ordem avançou para o Formulário 3. Crie ou continue uma ordem.
                @endif
            </p>
            <div class="flex flex-col sm:flex-row gap-3 justify-center items-center">
                ...
                @endif
                @if ($hasPermissionToCreate)
                    <a href="{{ route('company.orders.form3') }}" class="px-5 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all duration-300 flex items-center shadow-sm">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                        </svg>
                        Continuar Ordem
    ...
    {{-- Loading Overlay --}}
    <div wire:loading.flex wire:target="search,filterByOrderNumber,filterByMachineNumber,filterByStatus,filterByLocation,filterStartDate,filterEndDate,sortBy,continueOrder,viewOrder,refreshData,exportOrders" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                window.addEventListener('show-order-details', function(event) {
                    const data = event.detail[0].form3Data;
                    ...
                    modal.innerHTML = `
                        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-lg w-full shadow-xl max-h-[80vh] overflow-y-auto">
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">Detalhes da Ordem #${data.repair_order?.order_number|| data.id}</h3>
                            <div class="space-y-4 text-sm text-gray-700 dark:text-gray-300">
                                ...
                                <p><strong>Data Faturação:</strong> ${data.data_faturacao ? new Date(data.data_faturacao).toLocaleDateString('pt-BR') : '-'}</p>
                                <p><strong>Localização:</strong> ${data.location?.name || '-'}</p>
                                <p><strong>Status:</strong> ${data.status?.name || '-'}</p>
                                <p><strong>Horas Faturadas:</strong> ${data.horas_faturadas ? Number(data.horas_faturadas).toFixed(2) + 'h' : '-'}</p>
                                <p><strong>Materiais:</strong> ${data.materials?.length ? data.materials.map(m => m.name + ' (' + m.quantidade + ')').join(', ') : '-'}</p>
                                <p><strong>Nº Máquina:</strong> ${data.repairOrder?.form1.machineNumber.number || '-'}</p>
                            </div>
