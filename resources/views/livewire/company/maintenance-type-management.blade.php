<div class="space-y-6">
        ...
        <div class="mt-6 grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-lg p-4">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        <p class="text-2xl font-bold">{{ number_format($this->stats['avg_hourly_rate'], 2) }} MZN</p>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
        <div class="p-6">
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Taxa Mínima (MZN)</label>
                    <input wire:model.live.debounce.300ms="rateRangeMin" type="number" step="0.01" min="0"
                        class="w-full px-3 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent dark:bg-gray-700 dark:text-white"
                        placeholder="Ex: 100">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Taxa Máxima (MZN)</label>
                    <input wire:model.live.debounce.300ms="rateRangeMax" type="number" step="0.01" min="0"
                        class="w-full px-3 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent dark:bg-gray-700 dark:text-white"
                        placeholder="Ex: 1000">
                </div>
            </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-b-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <table class="w-full">
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Cor</th>
                        <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Descrição</th>
                        <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                            <button wire:click="sortBy('hourly_rate_mzn')" class="flex items-center space-x-1 hover:text-gray-700 dark:hover:text-gray-100">
                        ...
                        <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                            <button wire:click="sortBy('hourly_rate_usd')" class="flex items-center space-x-1 hover:text-gray-700 dark:hover:text-gray-100">
                        ...
                        <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Custos de Clientes</th>
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($maintenanceTypes as $type)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                            <td class="px-4 py-4">
                                <input type="checkbox" wire:model="selectedTypes.{{ $type->id }}" class="h-4 w-4 text-teal-600 focus:ring-teal-500 border-gray-300 rounded">
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <div class="h-10 w-10 rounded-lg flex items-center justify-center" style="background-color: {{ $type->color }}">
                                            <span class="text-white font-bold text-sm">{{ strtoupper(substr($type->name, 0, 1)) }}</span>
                                        </div>
                                    </div>
                                    <div class="ml-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="h-5 w-5 rounded-full mr-2" style="background-color: {{ $type->color }}"></div>
                                    <span class="text-sm text-gray-500 dark:text-gray-400">{{ $this->getColorName($type->color) }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900 dark:text-white">{{ number_format($type->hourly_rate_mzn, 2) }} MZN</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900 dark:text-white">{{ number_format($type->hourly_rate_usd, 2) }} USD</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <tr>
                            <td colspan="9" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">Nenhum tipo de manutenção encontrado</h3>
                                    <p class="text-gray-500 dark:text-gray-400 mb-4">Comece criando o seu primeiro tipo de manutenção ou use os tipos padrão.</p>
            ...
            <div class="bg-gray-50 dark:bg-gray-700 px-6 py-4 border-t border-gray-200 dark:border-gray-600">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button wire:click="bulkDeactivate({{ json_encode(array_keys(array_filter($selectedTypes))) }})"
        ...
        @if ($maintenanceTypes->hasPages())
            <div class="bg-gray-50 dark:bg-gray-700 px-6 py-4 border-t border-gray-200 dark:border-gray-600">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-700 dark:text-gray-300">
                        Mostrando {{ $maintenanceTypes->firstItem() }} a {{ $maintenanceTypes->lastItem() }} de {{ $maintenanceTypes->total() }} resultados
                    </div>
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                ...
                                <div class="mt-4">
                                    <form wire:submit.prevent="save" class="space-y-4">
                                        ...
                                        <div>
                                            <label for="default_hourly_rate_mzn" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Taxa Horária Padrão (MZN)</label>
                                            <input wire:model.live="default_hourly_rate" id="default_hourly_rate_mzn" type="number" step="0.01" min="0"
                                                class="mt-1 w-full px-3 py-2 border {{ $errors->has('default_hourly_rate') ? 'border-red-300' : 'border-gray-300 dark:border-gray-600' }} rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent dark:bg-gray-700 dark:text-white"
                                                placeholder="Ex: 500.00">
                                            @error('default_hourly_rate') <span class="text-sm text-red-600 dark:text-red-400">{{ $message }}</span> @enderror
                                        </div>
                                        <div>
                                            <label for="color" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Cor</label>
                                            <div class="mt-1 flex items-center gap-2">
                                                <select wire:model.live="color" id="color"
                                                    class="w-full px-3 py-2 border {{ $errors->has('color') ? 'border-red-300' : 'border-gray-300 dark:border-gray-600' }} rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent dark:bg-gray-700 dark:text-white">
                                                    @foreach ($availableColors as $hex => $name)
                                                        <option value="{{ $hex }}">{{ $name }}</option>
                                                    @endforeach
                                                </select>
                                                <button wire:click="generateRandomColor" type="button"
                                                    class="inline-flex items-center px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-teal-50 dark:hover:bg-teal-900/20 transition-colors">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                                                    </svg>
                                                </button>
                                            </div>
                                            <div class="mt-2 h-8 w-8 rounded-full" style="background-color: {{ $color }}"></div>
                                            @error('color') <span class="text-sm text-red-600 dark:text-red-400">{{ $message }}</span> @enderror
                                        </div>
                                        <div>
                                            <label class="flex items-center">
                                                ...
                                                <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Tipo ativo</span>
                                            </label>
                                        </div>
                                    </form>
                                </div>
                ...
                <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                                        ...
                                        <table class="w-full">
                                            <thead class="bg-gray-50 dark:bg-gray-700">
                                                <tr>
                                                    ...
                                                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Taxa (MZN)</th>
                                                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Taxa (USD)</th>
                                                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Data Efetiva</th>
                                                </tr>
                                            </thead>
                                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                                @forelse($this->clientRates as $rate)
                                                    <tr>
                                                        <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $rate->client->name }}</td>
                                                        <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ number_format($rate->cost_mzn, 2) }} MZN</td>
                                                        <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ number_format($rate->cost_usd, 2) }} USD</td>
                                                        <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $rate->effective_date->format('d/m/Y') }}</td>
                                                    </tr>
                                                @empty
                                                    <tr>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <button wire:click="closeRatesModal" type="button"
                            class="w-full inline-flex justify-center rounded-md border border-gray-300 dark:border-gray-600 shadow-sm px-4 py-2 bg-white dark:bg-gray-600 text-base font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 sm:w-auto sm:text-sm transition-colors">
                            Fechar
                        </button>
                    </div>
                </div>
                ...
                <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white" id="modal-title">
                                    Eliminar tipo de manutenção
                                </h3>
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500 dark:text-gray-400">
                                        Tem certeza que deseja eliminar este tipo de manutenção? Esta ação não pode ser desfeita.
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        ...
                        <button wire:click="$set('showDeleteModal', false)" type="button"
                            class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 dark:border-gray-600 shadow-sm px-4 py-2 bg-white dark:bg-gray-600 text-base font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 sm:mt-0 sm:mr-3 sm:w-auto sm:text-sm transition-colors">
                            Cancelar
                        </button>
                    </div>
                </div>
    ...
    <div wire:loading.delay class="fixed bottom-4 right-4 z-50">
        <div class="bg-teal-600 text-white px-4 py-2 rounded-lg shadow-lg flex items-center">
    ...
    @if (session()->has('success'))
        <div x-data="{ show: true }" x-show="show" x-transition:enter="transition ease-out duration-300"
            ...
            class="fixed top-4 right-4 z-50 bg-teal-500 text-white px-6 py-4 rounded-lg shadow-lg max-w-sm">
            <div class="flex items-center">
