<div class="space-y-6">
                ...
                <tbody class="bg-white dark:bg-zinc-800 divide-y divide-zinc-200 dark:divide-zinc-700">
                    @forelse($billings as $billing)
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-zinc-900 dark:text-white">
                                {{ number_format($billing->estimated_hours, 1) }}h
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                ...
                                <!-- Botão Alterar Moeda -->
                                <button wire:click="openEditModal({{ $billing->id }})"
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="relative inline-block align-bottom bg-white dark:bg-zinc-800 rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full sm:p-6">
                    ...
                    <div class="mt-6 flex justify-end space-x-3">
                        <button wire:click="openEditModal({{ $viewBilling->id }})"
    ...
    <!-- Modal de Alteração de Moeda (CORRIGIDO) -->
     <!-- Modal de Edição de Preços e Moeda -->
    @if($showEditModal && $selectedBilling)
        <div class="fixed inset-0 z-[60] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">

                <!-- Background overlay -->
                <div class="fixed inset-0 bg-zinc-700 bg-opacity-75 transition-opacity" wire:click="closeEditModal"></div>
                <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>

                <!-- Modal panel -->
                <div class="relative inline-block align-bottom bg-white dark:bg-zinc-800 rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full sm:p-6">
                    ...
                    <div class="flex items-center mb-6">
                        ...
                        <div class="ml-4">
                            <h3 class="text-lg leading-6 font-medium text-zinc-900 dark:text-white">Editar Faturação Estimada</h3>
                            <p class="text-sm text-zinc-500 dark:text-zinc-400">Ordem: <strong>{{ $selectedBilling->repairOrder->order_number }}</strong></p>
                        </div>
                    </div>

                    <!-- Informação da Taxa de Câmbio -->
                    <div class="mb-6 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                        <div class="flex items-center">
                            ...
                            <span class="text-sm text-blue-800 dark:text-blue-200">
                                <strong>Taxa de Câmbio:</strong> 1 USD = {{ $exchangeRate }} MZN (conversão automática)
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <!-- Valores Atuais -->
                        <div class="bg-zinc-50 dark:bg-zinc-700 rounded-lg p-4">
                            <h4 class="text-sm font-medium text-zinc-900 dark:text-white mb-3">Valores Atuais:</h4>
                            <div class="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    <span class="text-zinc-600 dark:text-zinc-400">Horas:</span>
                                    <span class="font-medium text-zinc-900 dark:text-white ml-2">{{ number_format($selectedBilling->estimated_hours, 1) }}h</span>
                                </div>
                                <div>
                                    <span class="text-zinc-600 dark:text-zinc-400">Moeda:</span>
                                    <span class="font-medium text-zinc-900 dark:text-white ml-2">{{ $selectedBilling->billing_currency }}</span>
                                </div>
                                <div>
                                    <span class="text-zinc-600 dark:text-zinc-400">Preço/h MZN:</span>
                                    <span class="font-medium text-zinc-900 dark:text-white ml-2">{{ number_format($selectedBilling->hourly_price_mzn, 2) }}</span>
                                </div>
                                <div>
                                    <span class="text-zinc-600 dark:text-zinc-400">Preço/h USD:</span>
                                    <span class="font-medium text-zinc-900 dark:text-white ml-2">$ {{ number_format($selectedBilling->hourly_price_usd, 2) }}</span>
                                </div>
                            </div>
                        </div>

                        <!-- Edição de Horas Estimadas -->
                        <div>
                            <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">Horas Estimadas</label>
                            <input type="number" 
                                   wire:model="newEstimatedHours" 
                                   step="0.1" 
                                   placeholder="Ex: 8.5"
                                   class="w-full px-3 py-2 border border-zinc-300 dark:border-zinc-600 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent dark:bg-zinc-700 dark:text-white">
                            @error('newEstimatedHours')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>

                        <!-- Preços por Hora (com conversão automática) -->
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">Preço por Hora (MZN)</label>
                                <input type="number" 
                                       wire:model.live="newHourlyPriceMzn" 
                                       step="0.01" 
                                       min="0"
                                       placeholder="Ex: 1000.00"
                                       class="w-full px-3 py-2 border border-zinc-300 dark:border-zinc-600 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent dark:bg-zinc-700 dark:text-white">
                                @error('newHourlyPriceMzn')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">Preço por Hora (USD)</label>
                                <input type="number" 
                                       wire:model.live="newHourlyPriceUsd" 
                                       step="0.01" 
                                       min="0"
                                       placeholder="Ex: 15.62"
                                       class="w-full px-3 py-2 border border-zinc-300 dark:border-zinc-600 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent dark:bg-zinc-700 dark:text-white">
                                @error('newHourlyPriceUsd')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                        </div>
                        ...
                        <!-- Seleção de Moeda -->
                        <div>
                            <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">Moeda de Faturação</label>
                            <div class="space-y-3">
                                <div class="flex items-center">
                                    <input id="currency-mzn-est" 
                                           type="radio" 
                                           wire:model="newCurrency" 
                                           value="MZN" 
                                           name="currency-estimated"
                                           class="h-4 w-4 text-orange-600 focus:ring-orange-500 border-zinc-300 dark:border-zinc-600">
                                    <label for="currency-mzn-est" class="ml-3 block text-sm font-medium text-zinc-700 dark:text-zinc-300">
                                        Metical (MZN)
                                    </label>
                                </div>
                                <div class="flex items-center">
                                    <input id="currency-usd-est" 
                                           type="radio" 
                                           wire:model="newCurrency" 
                                           value="USD" 
                                           name="currency-estimated"
                                           class="h-4 w-4 text-orange-600 focus:ring-orange-500 border-zinc-300 dark:border-zinc-600">
                                    <label for="currency-usd-est" class="ml-3 block text-sm font-medium text-zinc-700 dark:text-zinc-300">
                                        Dólar (USD)
                                    </label>
                                </div>
                            </div>
                        ...
                        <!-- Preview dos Totais -->
                        @if($newEstimatedHours > 0 && ($newHourlyPriceMzn > 0 || $newHourlyPriceUsd > 0))
                        <div class="bg-orange-50 dark:bg-orange-900/20 rounded-lg p-4 border border-orange-200 dark:border-orange-800">
                            <h4 class="text-sm font-medium text-orange-900 dark:text-orange-100 mb-2">Preview dos Novos Valores:</h4>
                            <div class="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    <span class="text-orange-700 dark:text-orange-300">Total MZN:</span>
                                    <span class="font-medium text-orange-900 dark:text-orange-100 ml-2">
                                        {{ number_format($newEstimatedHours * $newHourlyPriceMzn, 2) }} MZN
                                    </span>
                                </div>
                                <div>
                                    <span class="text-orange-700 dark:text-orange-300">Total USD:</span>
                                    <span class="font-medium text-orange-900 dark:text-orange-100 ml-2">
                                        $ {{ number_format($newEstimatedHours * $newHourlyPriceUsd, 2) }}
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Botões -->
                    <div class="mt-6 flex justify-end space-x-3">
                        <button wire:click="updateBilling" type="button"
                            class="inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-orange-600 text-base font-medium text-white hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition-colors">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button wire:click="closeEditModal" type="button"
                            class="inline-flex justify-center rounded-md border border-zinc-300 dark:border-zinc-600 shadow-sm px-4 py-2 bg-white dark:bg-zinc-800 text-base font-medium text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition-colors">
                            Cancelar
                        </button>
                    </div>
                </div>
