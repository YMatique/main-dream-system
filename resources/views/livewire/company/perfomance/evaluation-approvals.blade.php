<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <p class="text-gray-600 dark:text-gray-400 mt-1">Gerir aprovações de avaliações de desempenho - Sistema Multi-Estágio</p>
            </div>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-3 gap-4 mt-6 lg:mt-0">
                <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl border border-blue-200 dark:border-blue-800">
                    ...
                    <div class="text-2xl font-bold text-blue-700 dark:text-blue-300">{{ $stats['pending_for_me'] ?? 0 }}</div>
                </div>
                <div class="bg-red-50 dark:bg-red-900/20 p-4 rounded-xl border border-red-200 dark:border-red-800">
                    ...
                    <div class="text-2xl font-bold text-red-700 dark:text-red-300">{{ $stats['critical_for_me'] ?? 0 }}</div>
                </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        ...
        {{-- Ações rápidas --}}
        <div class="flex flex-wrap items-center justify-between gap-4 mt-4 pt-4 border-t border-gray-200 dark:border-gray-600">
    ...
    <div class="space-y-4">
        @if($evaluations->count() > 0)
            @foreach($evaluations as $evaluation)
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm hover:shadow-md transition-shadow {{ $evaluation->is_below_threshold ? 'ring-2 ring-red-200 dark:ring-red-800' : '' }}">
                    <div class="p-6">
                            ...
                            <div class="flex-1">
                                <div class="flex items-center gap-4 mb-4">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if($evaluation->is_below_threshold)
                                            <span class="flex items-center gap-1 px-2 py-1 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 text-xs font-medium rounded-full">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-600 dark:text-gray-400">{{ $evaluation->evaluation_period_formatted }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-gray-700 dark:text-gray-300">Avaliador</p>
                                        <p class="text-sm text-gray-600 dark:text-gray-400">{{ $evaluation->evaluator->name ?? 'N/A' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-gray-700 dark:text-gray-300">Submetida</p>
                                        <p class="text-sm text-gray-600 dark:text-gray-400">{{ $evaluation->submitted_at?->format('d/m/Y H:i') ?? 'N/A' }}</p>
                                    </div>
                                </div>
                                            ...
                                            <div class="flex items-center justify-between">
                                                <div>
                                                    <p class="text-sm font-medium text-blue-900 dark:text-blue-200">
                                                        Estágio {{ $stageInfo['stage_number'] }}: {{ $stageInfo['stage_name'] }}
                                                    </p>
                                                ...
                                                @if($stageInfo['is_my_turn'])
                                                    <span class="px-3 py-1 bg-blue-600 text-white text-xs font-medium rounded-full animate-pulse">
                                                        Sua vez
                                                    </span>
                                                @else
        ...
        @else
            ...
            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm">
                <div class="text-center py-16">
                    ...
                    <h3 class="text-xl font-medium text-gray-900 dark:text-white mb-3">Nenhuma avaliação encontrada</h3>
            ...
            <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200 dark:border-gray-600">
                <div>
                    ...
                    <p class="text-gray-600 dark:text-gray-400 mt-1">
                        {{ $selectedEvaluation->employee->name }} - {{ $selectedEvaluation->evaluation_period_formatted }}
                    </p>
                </div>
                <button wire:click="closeApprovalModal" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="font-medium text-green-900 dark:text-green-200">
                                Performance: {{ number_format($selectedEvaluation->final_percentage, 1) }}% - {{ $selectedEvaluation->performance_class }}
                            </p>
                            @if($selectedEvaluation->status === 'in_approval')
                                @php $stageInfo = $this->getCurrentStageInfo($selectedEvaluation); @endphp
                                @if($stageInfo)
                                    <p class="text-sm text-green-700 dark:text-green-300">
                                        Aprovando: {{ $stageInfo['stage_name'] }} (Estágio {{ $stageInfo['stage_number'] }})
                                    </p>
                                @endif
                            @endif
                        </div>
    ...
    <div class="fixed inset-0 z-50 overflow-y-auto bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-2xl w-full">
            ...
            <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200 dark:border-gray-600">
                ...
                <button wire:click="closeRejectionModal" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                    </svg>
                </button>
    ...
    <div class="fixed inset-0 z-50 overflow-y-auto bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-2xl w-full">
            ...
            <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200 dark:border-gray-600">
                ...
                <button wire:click="closeBulkModal" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                    </svg>
                </button>
    ...
    <div class="fixed inset-0 z-50 overflow-y-auto bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            {{-- Header --}}
            <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200 dark:border-gray-600 sticky top-0 bg-white dark:bg-gray-800 z-10">
                <div>
                ...
                <button wire:click="closeDetailModal" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                    </svg>
                </button>
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="bg-slate-50 dark:bg-gray-700/50 rounded-xl p-4">
                        <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-3">Informações do Funcionário</h4>
                    ...
                    <div class="bg-slate-50 dark:bg-gray-700/50 rounded-xl p-4">
                        <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-3">Resultado da Avaliação</h4>
                        <div class="space-y-2 text-sm">
                ...
                <div class="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-6">
                    <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Progresso de Aprovação</h4>
                    <div class="space-y-3">
                        @foreach($selectedEvaluation->approvals as $approval)
                            <div class="flex items-center justify-between p-3 bg-white dark:bg-gray-700 rounded-lg">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="font-medium text-gray-900 dark:text-white">{{ $approval->stage_name }}</p>
                                        <p class="text-sm text-gray-600 dark:text-gray-400">{{ $approval->approver->name ?? 'N/A' }}</p>
                                    </div>
                                </div>
                ...
                <div>
                    <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Respostas das Métricas</h4>
                    <div class="bg-white dark:bg-gray-700 rounded-xl overflow-hidden">
                        <div class="overflow-x-auto">
                            <table class="w-full">
                                <thead class="bg-gray-50 dark:bg-gray-800">
                                    <tr>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Métrica</th>
                                        ...
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Comentários</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                                    @foreach($selectedEvaluation->responses as $response)
                                    <tr>
                                        <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ $response->metric->name ?? 'N/A' }}</td>
                                        <td class="px-4 py-3 text-gray-600 dark:text-gray-400">{{ $response->display_value }}</td>
                                        <td class="px-4 py-3 text-gray-900 dark:text-white font-medium">{{ number_format($response->calculated_score, 1) }}</td>
                                        <td class="px-4 py-3 text-gray-600 dark:text-gray-400">{{ $response->comments ?: '-' }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                ...
                <div>
                    ...
                    <div class="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4">
                        <p class="text-gray-700 dark:text-gray-300">{{ $selectedEvaluation->recommendations }}</p>
                    </div>
                </div>
                @endif
            </div>
            ...
            {{-- Actions Footer --}}
            <div class="flex flex-col sm:flex-row gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50">
