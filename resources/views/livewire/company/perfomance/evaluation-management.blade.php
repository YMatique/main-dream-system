    ...
    <div class="bg-white rounded-lg shadow">
        ...
        <div class="p-4 border-b bg-gray-50">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <input wire:model.live="periodFilter" 
                       type="month" 
                       class="border rounded px-3 py-2">
                    ...
                    @forelse($evaluations as $evaluation)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3">
                                ...
                                <div class="text-sm text-gray-600">{{ $evaluation->employee->employee_code ?? 'N/A' }}</div>
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                ...
                                <div class="text-xs text-gray-500">{{ $evaluation->created_at->format('d/m/Y') }}</div>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center">
                                    <span class="text-sm font-medium mr-2">{{ number_format($evaluation->final_percentage, 1) }}%</span>
                                ...
                                <span class="px-2 py-1 rounded text-sm
                                    {{ $evaluation->status === 'approved' ? 'bg-green-100 text-green-800' : 
                                       ($evaluation->status === 'rejected' ? 'bg-red-100 text-red-800' : 
                                        ($evaluation->status === 'in_approval' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800')) }}">
    ...
    @if($showEvaluationModal)
        <div class="fixed inset-0 z-50 overflow-y-auto">
            <div class="flex items-center justify-center min-h-screen px-4">
                <div class="fixed inset-0 bg-gray-900 bg-opacity-50" wire:click="closeEvaluationModal"></div>
                        ...
                        <div class="p-6 space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input wire:model.live="evaluationPeriod" 
                                           type="month" 
                                           class="w-full border rounded px-3 py-2"
                                           {{ $currentEvaluationId ? 'disabled' : '' }}>
                                    ...
                                    <div class="border-t pt-6">
                                        ...
                                        @foreach($metrics as $metric)
                                            <div class="bg-gray-50 rounded-lg p-4 mb-4">
                                                <div class="flex justify-between items-start mb-3">
                                                    <div class="flex-1">
                                                        ...
                                                        @if($metric->description)
                                                            <p class="text-sm text-gray-600 mt-1">{{ $metric->description }}</p>
                                                        @endif
                                                    </div>
                                                ...
                                                @if($metric->type === 'numeric')
                                                    <div class="mb-3">
                                                        <label class="block text-sm font-medium mb-1">
                                                            Pontuação ({{ $metric->min_value }} - {{ $metric->max_value }}) *
                                                        </label>
                                                        <input wire:model.live="responses.{{ $metric->id }}.numeric_value" 
                                                               type="number" 
                                                               step="0.1"
                                                ...
                                                <div>
                                                    <label class="block text-sm font-medium mb-1">Comentários (opcional)</label>
                                                    <textarea wire:model="responses.{{ $metric->id }}.comments" 
                                                              class="w-full border rounded px-3 py-2" 
                                                              rows="2"
                                    ...
                                    <div class="bg-blue-50 rounded-lg p-4">
                                        <div class="flex justify-between items-center">
                                            <div>
                                                ...
                                                <p class="text-sm text-gray-600">Baseado nas métricas preenchidas</p>
                                            </div>
                                            <div class="text-right">
                                                <div class="text-2xl font-bold {{ $finalPercentage >= 70 ? 'text-green-600' : ($finalPercentage >= 50 ? 'text-yellow-600' : 'text-red-600') }}">
                                        ...
                                        @if($finalPercentage < 50)
                                            <div class="mt-3 p-3 bg-red-100 border border-red-200 rounded">
                                                <div class="flex">
                                                    ...
                                                    <div>
                                                        <h6 class="text-sm font-medium text-red-800">Baixo Desempenho Detectado</h6>
                                                        <p class="text-sm text-red-700">Esta avaliação ficará abaixo do limite de 50% e será enviada notificação automática.</p>
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
                            ...
                            @elseif($selectedEmployeeId && $selectedDepartmentId)
                                <div class="text-center py-8">
                                    ...
                                    <h4 class="text-lg font-medium text-gray-900 mb-2">Métricas não configuradas</h4>
                                    <p class="text-gray-600 mb-4">
                                        O departamento selecionado não possui métricas configuradas ou o peso total não soma 100%.
                                    </p>
                    ...
                    <div class="bg-white rounded-lg shadow-xl w-full max-w-4xl relative max-h-screen overflow-y-auto">
                        <div class="p-6 border-b">
                            <div class="flex justify-between items-start">
                                <div>
                                    <h3 class="text-lg font-medium">Avaliação de {{ $viewEvaluation->employee->name }}</h3>
                                    <p class="text-sm text-gray-600">
                                        {{ $viewEvaluation->employee->department->name }} - {{ $viewEvaluation->evaluation_period->format('m/Y') }}
                                    </p>
                                </div>
                                <span class="px-3 py-1 rounded text-sm
                                    {{ $viewEvaluation->status === 'approved' ? 'bg-green-100 text-green-800' : 
                                       ($viewEvaluation->status === 'rejected' ? 'bg-red-100 text-red-800' : 
                                        ($viewEvaluation->status === 'in_approval' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800')) }}">
                        ...
                        <div class="p-6 space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div class="bg-gray-50 rounded p-4">
                                    ...
                                    <div class="text-2xl font-bold {{ $viewEvaluation->final_percentage >= 70 ? 'text-green-600' : ($viewEvaluation->final_percentage >= 50 ? 'text-yellow-600' : 'text-red-600') }}">
                                        {{ number_format($viewEvaluation->final_percentage, 1) }}%
                                    </div>
                                    <div class="text-sm text-gray-600">{{ $viewEvaluation->performance_class }}</div>
                                </div>

                                <div class="bg-gray-50 rounded p-4">
                                    <h5 class="font-medium mb-2">Avaliador</h5>
                                    <div class="text-sm">{{ $viewEvaluation->evaluator->name }}</div>
                                    <div class="text-xs text-gray-500">{{ $viewEvaluation->created_at->format('d/m/Y H:i') }}</div>
                                </div>
                                ...
                                <div class="bg-gray-50 rounded p-4">
                                    ...
                                    @if($viewEvaluation->approved_at)
                                        <div class="text-xs text-gray-500">Aprovada em {{ $viewEvaluation->approved_at->format('d/m/Y') }}</div>
                                    @endif
                                </div>
                            </div>
                                    ...
                                    @foreach($viewEvaluation->responses as $response)
                                        <div class="bg-gray-50 rounded p-4">
                                            <div class="flex justify-between items-start mb-2">
                                                <div>
                                                    ...
                                                    @if($response->metric->description)
                                                        <p class="text-sm text-gray-600">{{ $response->metric->description }}</p>
                                                    @endif
                                                </div>
                                                <div class="text-right">
                                                    <span class="px-2 py-1 bg-blue-100 text-blue-800 rounded text-sm">
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div>
                                                    <label class="block text-sm font-medium mb-1">Resposta</label>
                                                    <div class="text-lg font-medium">{{ $response->display_value }}</div>
                                                </div>
                                                <div>
                                                    <label class="block text-sm font-medium mb-1">Score Calculado</label>
                                                    <div class="text-lg font-medium">{{ number_format($response->calculated_score, 2) }} pontos</div>
                                                </div>
                                            </div>

                                            @if($response->comments)
                                                <div class="mt-3">
                                                    <label class="block text-sm font-medium mb-1">Comentários</label>
                                ...
                                <div class="bg-red-50 border border-red-200 rounded p-4">
                                    <div class="flex">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-red-700">Esta avaliação está abaixo do limite de 50% e notificações foram enviadas.</p>
                                        </div>
                                    </div>
                                </div>
                            @endif
