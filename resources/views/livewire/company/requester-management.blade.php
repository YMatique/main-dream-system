<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-teal-500 via-blue-600 to-indigo-700 rounded-xl shadow-lg p-6 text-white">
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center">
            <div>
                <h2 class="text-3xl font-bold tracking-tight">Gestão de Solicitantes</h2>
                <p class="mt-2 text-teal-100">Gerencie os solicitantes de ordens de reparação da sua empresa</p>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        ...
                        Pesquisar solicitantes
                    </label>
                    <div class="relative">
                        <input wire:model.live.debounce.300ms="search" type="text"
                            class="w-full pl-10 pr-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-1 focus:ring-teal-500 focus:border-transparent dark:bg-gray-700 dark:text-white"
    ...
    <div class="sticky top-0 z-10 bg-white dark:bg-gray-800 rounded-t-xl shadow border border-gray-200 dark:border-gray-700 p-4 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="flex items-center gap-2">
            ...
            <button wire:click="export('excel')"
                class="inline-flex items-center px-4 py-2 bg-white dark:bg-zinc-700 text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-teal-50 dark:hover:bg-teal-900/20 transition-colors">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <button wire:click="bulkDeactivate"
                class="inline-flex items-center px-4 py-2 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-teal-50 dark:hover:bg-teal-900/20 transition-colors"
                title="Desativar solicitantes inativos sem ordens ativas">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="text-sm text-gray-600 dark:text-gray-400">
            {{ $requesters->total() }} solicitantes encontrados
        </div>
    </div>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <div
                                            class="h-10 w-10 rounded-lg bg-gradient-to-br from-teal-500 to-blue-600 flex items-center justify-center">
                                            <span class="text-white font-bold text-sm">{{ strtoupper(substr($requester->name, 0, 1)) }}</span>
                                        </div>
                                    </div>
                                    <div class="ml-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-500 dark:text-gray-400">
                                    {{ $requester->email ?: 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if ($requester->active_orders_count > 0)
                                    <span
                                        ...
                                        {{ $requester->active_orders_count }}
                                    </span>
                                @else
                                    <span class="text-sm text-gray-500 dark:text-gray-400">Nenhuma</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <button wire:click="toggleStatus({{ $requester->id }})"
                                    class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium transition-colors {{ $requester->is_active ? 'bg-teal-100 text-teal-800 hover:bg-teal-200 dark:bg-teal-900 dark:text-teal-200' : 'bg-red-100 text-red-800 hover:bg-red-200 dark:bg-red-900 dark:text-red-200' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2">
                                    <button wire:click="edit({{ $requester->id }})"
                                    ...
                                    <button wire:click="confirmDelete({{ $requester->id }})"
                                        class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 transition-colors {{ $requester->active_orders_count > 0 ? 'opacity-50 cursor-not-allowed' : '' }}"
                                        title="{{ $requester->active_orders_count > 0 ? 'Não pode ser eliminado devido a ordens ativas' : 'Eliminar' }}"
                                        {{ $requester->active_orders_count > 0 ? 'disabled' : '' }}>
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">Nenhum solicitante encontrado</h3>
                                    <p class="text-gray-500 dark:text-gray-400 mb-4">Comece criando o seu primeiro solicitante.</p>
                                    <button wire:click="openModal"
            ...
            <div class="bg-gray-50 dark:bg-gray-700 px-6 py-4 border-t border-gray-200 dark:border-gray-600">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-700 dark:text-gray-300">
                        Mostrando {{ $requesters->firstItem() }} a {{ $requesters->lastItem() }} de
                        {{ $requesters->total() }} resultados
                    </div>
    ...
    {{-- Modal Criar/Editar Solicitante --}}
    @if ($showModal)
                                    ...
                                    <form wire:submit.prevent="save" class="space-y-4">
                                        <div>
                                            ...
                                            <input wire:model.live="name" id="name" type="text"
                                                class="mt-1 w-full px-3 py-2 border outline-0 {{ $errors->has('name') ? 'border-red-300' : 'border-gray-300 dark:border-gray-600' }} rounded-lg focus:ring-1 focus:ring-teal-500 ring-1 focus:border-transparent dark:bg-gray-700 dark:text-white"
                                        ...
                                        <div>
                                            ...
                                            <input wire:model.live="email" id="email" type="email"
                                                class="mt-1 w-full px-3 py-2 border outline-0 {{ $errors->has('email') ? 'border-red-300' : 'border-gray-300 dark:border-gray-600' }} rounded-lg focus:ring-2 focus:ring-teal-500 ring-1 focus:border-transparent dark:bg-gray-700 dark:text-white"
                                        ...
                                        <div>
                                            <label class="flex items-center">
                                                ...
                                                <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Solicitante ativo</span>
                                            </label>
                                        </div>
                                    </form>
                                </div>
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500 dark:text-gray-400">
                                        Tem certeza que deseja eliminar este solicitante? Esta ação não pode ser
                                        desfeita.
                                    </p>
                                </div>
                            </div>
    ...
    @if (session()->has('warning'))
        <div x-data="{ show: true }" x-show="show" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="flex items-center">
                ...
                <button @click="show = false" class="ml-4">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0  apostle: 4
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M6 18L18 6M6 6l12 12">
                    </path>
                </svg>
