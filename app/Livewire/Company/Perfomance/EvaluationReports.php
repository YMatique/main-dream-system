...
class EvaluationReports extends Component
{
    ...
    // Report Type (CORRIGIDO para activeTab)
    public $activeTab = 'overview'; // overview, performance, department, employee, trends
    ...
    // Para controle dos filtros avançados
    ...
    // Método para alternar filtros avançados
    ...
    public function exportReport()
    {
        ...
        switch ($this->exportFormat) {
            case 'xlsx':
                return Excel::download(new EvaluationReportExport($this->reportData, $this->activeTab), 
                    'relatorio-avaliacoes-' . $this->activeTab . '-' . now()->format('Y-m-d') . '.xlsx');
            case 'csv':
                return Excel::download(new EvaluationReportExport($this->reportData, $this->activeTab), 
                    'relatorio-avaliacoes-' . $this->activeTab . '-' . now()->format('Y-m-d') . '.csv', 
            ...
            case 'pdf':
                ...
                ])->download('relatorio-avaliacoes-' . $this->activeTab . '-' . now()->format('Y-m-d') . '.pdf');
        }
        */
    ...
    protected function optimizeQuery()
    {
        ...
        $cacheKey = 'report_' . $this->activeTab . '_' . md5(serialize([
            $this->startDate,
            $this->endDate,
